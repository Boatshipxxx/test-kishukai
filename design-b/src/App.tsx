...
const TARGETS: { id: Target; label: string; en: string }[] = [
...
const FOOTER_COLUMNS: { name: string; links: string[] }[] = [
  { name: 'ABOUT', links: ['私たちの理念', '五大精神と「情」', '指導の3本柱', '指導者紹介'] },
  { name: 'PROGRAM', links: ['キッズ', 'ベーシック', 'アクロバット', '選手育成', 'カーディオ'] },
  { name: 'WORLD', links: ['WTCE遠征レポート', '遠征の記録', '保護者の声', '国際交流'] },
  { name: 'JOIN', links: ['無料体験', '入会までの流れ', '料金プラン', 'よくある質問'] },
  { name: 'SUPPORT', links: ['一口協賛', '年間サポーター', '現物協賛', '地域活動実績'] },
  { name: 'CONTACT', links: ['お問い合わせ', 'アクセス', 'お知らせ', '現行サイト'] },
]
    ...
    <div className="min-h-screen bg-white text-black">
        ...
        <div className="mx-auto max-w-[1240px] flex items-center justify-between px-4 h-16">
          <a href="#top" className="flex items-center gap-3">
            ...
            <span className="leading-none">
              <span className="font-display text-2xl font-bold tracking-tight">輝蹴会</span>
          ...
          <dl className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-px bg-white/15 border border-white/15">
            {[
              ...
              <div key={label} className="bg-black px-4 py-4">
                <dt className="font-display text-[10px] tracking-[0.2em] text-gold">{label}</dt>
        ...
        <div className="grid gap-px bg-black/15 border border-black/15 md:grid-cols-3">
          {[
            ...
            <div key={item.k} className="bg-white p-7">
              ...
              <p className="mt-3 text-sm leading-relaxed text-black/60">{item.d}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
            <div>
              ...
              <div className="grid grid-cols-3 gap-px bg-white/15 border border-white/15 border-t-0">
                <Photo label="現地の仲間と" tone="blood" className="aspect-square" />
                <Photo label="演武の瞬間" tone="gold" className="aspect-square" />
                <Photo label="茂朱の会場にて" tone="navy" className="aspect-square" />
              </div>
            </div>
            <div>
              ...
              <dl className="mt-8 border-t border-white/15 text-sm">
                {[
                  ...
                  <div key={label} className="flex border-b border-white/15 py-3">
        ...
        <div className="border border-black">
          ...
          <div className="p-4 flex flex-wrap items-center gap-2">
            ...
            {TARGETS.map((t) => (
              ...
              >
                {t.en}
                <span className="ml-2 font-body text-xs normal-case opacity-70">{t.label}</span>
              </button>
            ))}
          </div>
        </div>
            ...
            {filtered.map((c) => (
              <article key={c.name} className="group bg-white flex flex-col">
                <div className="relative">
                  <Photo label={'写真差し替え：' + c.name} tone={c.tone} className="aspect-[3/2]" />
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <dl className="mt-4 text-sm border-t border-black/15">
                    {[
                      ['対象', TARGETS.find((t) => t.id === c.target)?.label ?? ''],
                      ['ねらい', c.who],
                      ['実施日時', c.schedule],
                      ['形式', c.style],
                      ['月謝', c.monthly],
      ...
      <section id="price" className="bg-fog border-y border-black/15">
        <div className="mx-auto max-w-[1240px] px-4 py-16">
          ...
          <div className="grid gap-px bg-black/15 border border-black/15 md:grid-cols-3">
            {PLANS.map((p) => (
              <div
                key={p.name}
                className={
                  'p-7 flex flex-col ' + (p.featured ? 'bg-black text-white' : 'bg-white')
                }
              >
          ...
          <div className="mt-px border border-black/15 border-t-0 bg-white">
            <dl className="text-sm">
              {[
                ...
                ['道着・防具', '入会後しばらくはレンタルで開始可。昇級に応じてご案内します。'],
              ].map(([label, value]) => (
                <div key={label} className="flex border-b border-black/15 last:border-b-0 px-5 py-3">
          ...
          <p className="mt-4 text-xs text-black/50">※ 金額はすべて差し替え項目です。</p>
        </div>
      </section>
      ...
      <section id="news" className="mx-auto max-w-[1240px] px-4 py-16">
        <div className="flex items-end justify-between border-b-2 border-black pb-3 mb-2">
          <div>
            <p className="font-display text-gold tracking-[0.3em] text-xs">NEWS</p>
            ...
            <div className="mt-7 grid gap-px bg-black/20 border border-black/20 sm:grid-cols-3">
              {[
                ...
                <div key={name} className="bg-gold p-4">
                  ...
                  <p className="font-display text-lg font-bold mt-1 tabular-nums">{price}</p>
        ...
        <div className="mx-auto max-w-[1240px] px-4 py-14">
          <div className="flex items-center gap-3 mb-8">
            <span className="font-display text-3xl font-bold tracking-tight">
              輝蹴会
              <span className="ml-2 bg-gold text-black px-1.5 text-xl align-middle">KISHUKAI</span>
            </span>
          </div>
